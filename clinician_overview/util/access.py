...
def has_access(client: Client, time: datetime.datetime | None = None) -> AccessGrant | None:
    """
    Return an AccessGrant that is currently active as proof that the client currently has access to STEP.
    """
    return _active_access_grants(client, time).first()

def new_grant_until(client: Client, end_date: datetime.datetime) -> AccessGrant:
    """
...
def has_access_until(client: Client) -> datetime.datetime | None:
    latest_date_in_active_access_grants: datetime.datetime | None = _active_access_grants(client) \
        .order_by("-expires_at") \
...
def _active_access_grants(client: Client, time: datetime.datetime | None = None) -> BaseManager[AccessGrant]:
    ...
    .filter(Q(client__is_active=True)) \
...
def revoke_current_access_for(clinician: User, client: Client) -> None:
    ...
    """
    active_grants = _active_access_grants(client)
    now = timezone.now()
...
def extend_access(clinician: User, client: Client, num_days: int) -> AccessGrant:
    ...
    new_grant = new_grant_until(client, end_date)
    new_grant.save()
    return new_grant
